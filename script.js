...
function showModal(meal) {
  ...
  for (let i = 1; i <= 20; i++) {
    ...
    if (ingredient && ingredient.trim() !== "") {
      ingredientsList +=
        "<li>" + ingredient + (measure ? " - " + measure : "") + "</li>";
    }
  }
  let youtubeHtml = "";
  if (meal.strYoutube) {
    youtubeHtml = `<p class="mt-2">
      <strong>Watch on YouTube:</strong>
      <a href="${meal.strYoutube}" target="_blank" rel="nooperner noreferrer">Click here</a>
    </p>`;
  }
  ...
  function toggleFavourite(id, title) {
    ...
    if (favourites[id]) {
      ...
      btn.innerText = "Mark as Favourite";
      btn.classList.replace("btn-danger", "btn-outline-danger");
    } else {
      // If not, add it with default empty notes
      favourites[id] = { title: title, notes: "" };
      btn.innerText = "Remove Favourite";
